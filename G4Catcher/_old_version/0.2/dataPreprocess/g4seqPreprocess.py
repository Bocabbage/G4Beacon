...
# def _reverse_seq(ifastaFile: str, ofastaFile: str) -> None:
#     with open(ofastaFile, 'w+') as ofile:
#         for record in SeqIO.parse(ifastaFile, "fasta"):
#             header, seq = str(record.id), str(record.seq)
#             seq = seq.upper()[::-1]
...
def g4_sequence_extract(bedFile: str,
    ...
    # if not reverse:
    #     run_shell_cmd("bedtools getfasta -fi {} -bed {} > {}".format(refGenomeFile, tmpBedFile, ofastaFile))
    # else: # For reverse strand
    #     tmpFaFile = "{}tmp.extendG4Rigion2.bed".format(randId)
    #     run_shell_cmd("bedtools getfasta -fi {} -bed {} > {}".format(refGenomeFile, tmpBedFile, tmpFaFile))
    #     _reverse_seq(tmpFaFile, ofastaFile)
    #     run_shell_cmd("rm {}".format(tmpFaFile))

    # run_shell_cmd("grep '>' {} | awk -F '[>:-]+' BEGIN{{ OFS=\"\t\" }}{{ print $2, $3, $4 }}' > {}".format(ofastaFile, obedFile))
