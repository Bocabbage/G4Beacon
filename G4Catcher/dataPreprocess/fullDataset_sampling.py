...
neg_size = int(subprocess.check_output(["wc", "-l", origin_data_file[3]]).decode('utf-8').split(" ")[0])
...
for config in json_data['config_lists']:
    ...
    # if config['pos_shuffle_list'] is not None:
    ...
    if mode == 'smote':
        ...
        train_feature = pd.concat([train_seq, train_atac], axis=1)
